...
- **`git-commit`** — обязательная конвенция оформления коммитов. Формат
  `<type>: <описание на русском>` (type: `feat`/`fix`/`refactor`/`hotfix`/`design`,
...
- **`deploy`** — особенности деплоя и прод-запуска на боевом сервере (shared-хостинг,
  nginx-конфликт с GravitLauncher на `/api`, pm2, права доступа, БД в docker).
  Читать перед любой работой с `docker-compose.yml`, `deploy.sh`, `.env`,
  nginx-конфигом сайта или pm2, а также при диагностике "сайт не поднялся".
